...
class AuthManager {
    ...
    async signIn(email, password) {
        console.log('🔐 Attempting login for:', email, 'Demo Mode:', shouldUseDemoMode);
        
        try {
            let userCredential;
            
            if (shouldUseDemoMode) {
                userCredential = await this.demoAuth.signInWithEmailAndPassword(email, password);
            } else {
                try {
                    ...
                    console.warn('Firebase auth failed, switching to demo mode:', firebaseError.message);
                    this.switchToDemoMode();
                    userCredential = await this.demoAuth.signInWithEmailAndPassword(email, password);
                }
            }
            ...
            // Create new session (this will override any existing session)
            await this.createSession();
            
            console.log('✅ Login successful:', user.email, 'Admin:', this.isAdmin());
    ...
    async signUp(email, password) {
        try {
            ...
            if (shouldUseDemoMode) {
                userCredential = await this.demoAuth.createUserWithEmailAndPassword(email, password);
            } else {
                try {
